...
from llm_integration import create_llm_manager, create_conversation_manager, create_rag_chain
...
class DocumentManager:
    ...
    def add_document_from_text(self, text: str, title: str = "User Text") -> Dict[str, Any]:
    ...
    def add_documents_from_directory(self, directory_path: str, 
        ...
        return {
            ...
            "failed": error_count,
            "results": results,
            "total_chunks_added": sum(r["result"].get("chunks_added", 0) for r in results if r["result"]["success"])
        }
...
class ConversationInterface:
    ...
    async def send_message(self, message: str, use_workflow: str = None) -> Dict[str, Any]:
        ...
        try:
            ...
            # Process message
            if use_workflow:
                result = await self.workflow_manager.execute_workflow(use_workflow, message)
            else:
...
class RAGApplication:
    ...
    async def initialize(self) -> Dict[str, Any]:
        ...
        try:
            ...
            retriever = create_retriever(self.document_manager.vector_store, self.config)
            ...
            self.conversation_interface = ConversationInterface(self.agent, self.workflow_manager)
    ...
    async def add_document(self, source: str, source_type: str = "file") -> Dict[str, Any]:
        ...
        while True:
            try:
                ...
                if command in ['quit', 'exit']:
                    ...
                    print(f"\n📊 Application Status:")
                    print(f"  📄 Documents: {status['document_stats']['total_documents']}")
                    print(f"  📝 Chunks: {status['document_stats']['total_chunks']}")
                    print(f"  🧠 Memory items: {status['memory_summary']['short_term_items']}")
                    print(f"  🛠️  Available tools: {len(status['agent_status']['tools'])}")
                    ...
                    if result["success"]:
                        print(f"✅ Added {result['filename']}: {result['chunks_added']} chunks")
                    else:
                    ...
                    if result["success"]:
                        print(f"✅ Added {result['successful']}/{result['total_files']} documents")
