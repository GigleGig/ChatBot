...
class GitHubSearchWithContentTool(Tool):
    ...
    async def execute(
        ...
        **kwargs,
        ...
        try:
            ...
            search_result = await self._perform_github_search(
                query, search_type, language, **kwargs
            )
            ...
            if search_result.get("items") and fetch_content:
                print(
                    f"📥 Fetching content for up to {max_content_files} files from {len(search_result['items'])} results..."
                )
                ...
                for i, item in enumerate(search_result["items"][:max_content_files]):
                    ...
                    if "repository" in item and "path" in item:
                        ...
                        if owner and repo_name and file_path:
                            print(
                                f"📦 Fetching content from {owner}/{repo_name}/{file_path}"
                            )
            ...
            return ToolResult(
                ...
                result={
                    ...
                    "files_with_content": len(
                        [r for r in results_with_content if r.get("content")]
                    ),
                },
                execution_time=execution_time,
                metadata={"language": language, "max_content_files": max_content_files},
            )
    ...
    async def _perform_github_search(
        self, query: str, search_type: str, language: Optional[str] = None, **kwargs
    ) -> Dict[str, Any]:
...
class GitHubSearchTool(Tool):
        ...
        async with aiohttp.ClientSession() as session:
            async with session.get(
                ...
                if response.status == 200:
                    ...
                    print(
                        f"📊 API returned {json_data.get('total_count', 0)} total results"
                    )
    ...
    async def _format_results(
        self, items: List[Dict[str, Any]], search_type: str
...
class GitHubCodeSearchTool(GitHubSearchTool):
    ...
    def __init__(self, github_token: Optional[str] = None):
        ...
        self.description = (
            "Search for code snippets, functions, and implementations on GitHub"
        )
    ...
    async def execute(
        ...
        **kwargs,
        ...
        print(
            f"🔍 Simplified search query: '{query}' (from original: '{kwargs.get('original_query', query)}')"
        )

        return await super().execute(
            query, language=language, repository=repository, **kwargs
        )
...
def create_github_code_search_tool(
...
def create_github_search_with_content_tool(
    github_token: Optional[str] = None,
...
async def test_github_search():
    ...
    result = await tool.execute(
        query="async def", search_type="code", language="python", per_page=5
    )
