...
class VectorStoreConfig(BaseModel):
    ...
    vector_store_type: str = Field(default="chroma", description="Type of vector store (chroma, faiss)")
    collection_name: str = Field(default="code_agent_docs", description="Collection name in vector store")
    persist_directory: str = Field(default="./vector_store", description="Directory to persist vector store")
    
    # Embedding Settings
    embedding_model: str = Field(default="text-embedding-ada-002", description="Embedding model name")
    chunk_size: int = Field(default=1000, gt=0, description="Text chunk size for processing")
    chunk_overlap: int = Field(default=200, ge=0, description="Overlap between text chunks")
    
    # Search Settings
    search_k: int = Field(default=5, gt=0, description="Number of documents to retrieve")
    search_type: str = Field(default="similarity", description="Type of search (similarity, mmr)")
...
class AgentConfig(BaseModel):
    ...
    max_execution_time: int = Field(default=60, gt=0, description="Maximum execution time in seconds")
    
    # Memory Settings
    memory_type: str = Field(default="buffer", description="Type of memory (buffer, summary)")
    memory_max_tokens: int = Field(default=2000, gt=0, description="Maximum tokens in memory")
    
    # Tool Settings
    enable_code_execution: bool = Field(default=False, description="Enable code execution tools")
    enable_file_operations: bool = Field(default=True, description="Enable file operation tools")
    enable_web_search: bool = Field(default=False, description="Enable web search tools")
...
class ProjectConfig(BaseModel):
    ...
    # Project Settings
    project_name: str = Field(default="Code Agent RAG System", description="Project name")
