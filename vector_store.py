...
class VectorStore(ABC):
    ...
    @abstractmethod
    def add_documents(self, chunks: List[DocumentChunk], metadata: Optional[DocumentMetadata] = None) -> bool:
...
class ChromaVectorStore(VectorStore):
    ...
    def __init__(self, collection_name: str = "rag_documents", 
                 persist_directory: str = "./chroma_db", config=None):
    ...
    def similarity_search(self, query: str, k: int = 5, **kwargs) -> List[SearchResult]:
        ...
        try:
            ...
            if results["documents"] and len(results["documents"]) > 0:
                ...
                for rank, (doc, meta, distance) in enumerate(zip(documents, metadatas, distances)):
    ...
    def delete_documents(self, document_ids: List[str]) -> bool:
        ...
        try:
            self.collection.delete(
                where={"source_document": {"$in": document_ids}}
            )
            return True
        except Exception as e:
...
class FAISSVectorStore(VectorStore):
    ...
    def similarity_search(self, query: str, k: int = 5, **kwargs) -> List[SearchResult]:
        ...
        if not query or not query.strip() or self.index is None or len(self.documents) == 0:
            return []
    ...
    def delete_documents(self, document_ids: List[str]) -> bool:
        ...
        print("Warning: FAISS doesn't support direct deletion. Consider rebuilding the index.")
        return False
...
class DocumentRetriever:
    ...
    def retrieve_documents(self, query: str, k: int = 5, min_score: float = 0.0) -> List[SearchResult]:
...
def create_retriever(vector_store: VectorStore = None, config=None, **kwargs) -> DocumentRetriever:
